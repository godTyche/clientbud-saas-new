    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('module_in_package', function ($row) use ($modulesAll) {
                ...
                $string .= '<span class="col-md-3">' . $sign . ' ' .'HR' . ($row->id == 1 ? '<br/><span class="module-description">'.$hr_desc.'</span><input class="module-desc" data-value="employees" type="text" value="'.$hr_desc.'" style="display:none"/>' : ''). '</span>';

                $sign = in_array($WORK_MODULES[0], $modules) ? ('<i class="fa fa-check"></i>') : ('<i class="fa fa-times"></i>');
                $string .= '<span class="col-md-3">' . $sign . ' ' .'Work' . ($row->id == 1 ? '<br/><span class="module-description">'.$work_desc.'</span><input class="module-desc" data-value="contracts" type="text" value="'.$work_desc.'" style="display:none"/>' : ''). '</span>';

                $sign = in_array($FINANCE_MODULES[0], $modules) ? ('<i class="fa fa-check"></i>') : ('<i class="fa fa-times"></i>');
                $string .= '<span class="col-md-3">' . $sign . ' ' .'Finance' . ($row->id == 1 ? '<br/><span class="module-description">'.$finance_desc.'</span><input class="module-desc" data-value="estimates" type="text" value="'.$finance_desc.'" style="display:none"/>' : ''). '</span>';
                ...
                foreach ($modulesAll as $module) {
                    ...
                    $string .= '<span class="col-md-3">' . $sign . ' ' . __('modules.module.' . $module->module_name) . ($row->id == 1 ? '<br/><span class="module-description">'.$module->module_description.'</span><input class="module-desc" data-value="'.$module->module_name.'" type="text" value="'.$module->module_description.'" style="display:none"/>' : ''). '</span>';
                }
